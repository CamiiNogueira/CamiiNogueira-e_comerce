    ...
    carrito.forEach(producto => {
        let fila = `
            <tr>
                ...
                <td>
                    <div "input-group me-2" style="max-width: 150px;">
                        <button id="btnMenos-${producto.id}" class="btn btn-outline-secondary">-</button>
                        <input type="text" value="${producto.cantidad}" id="quantity-${producto.id}" style="width: 45px; text-align: center;">
                        <button id="btnMas-${producto.id}" class="btn btn-outline-secondary">+</button>
                    </div>
                </td>
                    <td>${producto.moneda} ${producto.precio * producto.cantidad}</td>
